...
private:
	...
	void sort_split(const int_iter& p, int64_t n)
	{
		...
		while (1) {			 /* split-end partition.*/
			while (pb <= pc &&
				   (f = key(pb)) <=
				   v) { // go to the right as long as there are keys smaller equal than v
				if (f == v) {
					swap(pa, pb);
			...
			while (pc >= pb &&
				   (f = key(pc)) >=
				   v) { // go to the left as long as there are keys bigger or equal to v
				if (f == v) {
					swap(pc, pd);
...
public:
	...
	int64_t
	transform(const int_iter& x, const int_iter& p, int64_t n, int64_t k, int64_t l, int64_t q)
	{
		if (!(q >= k - l)) {
			std::cout << "q=" << q << " k-l=" << k - l << std::endl;
		}
		assert(q >= k - l);
		DBG_OUT << "transform(n=" << n << ", k=" << k << ", l=" << l << ", q=" << q << ")"
				<< std::endl;
		uint64_t bb, cc, dd;
		int64_t  jj;
		int_iter pi, pj;
		...
		m_rr		 = 0;
		for (bb = dd = 0;
			 (int)m_rr < n && (int)len < m_msb + 1 - s && (int64_t)(cc = dd << s | (k - l)) <= q;
			 ++m_rr, len += s) {
	...
	void do_sort(tIV& sa, tIV& x)
	{
		...
		int64_t max_symbol = 0,
				min_symbol = x.width() < 64 ? bits::lo_set[x.width()] : 0x7FFFFFFFFFFFFFFFLL;
		...
		DBG_OUT << "sorter: width=" << (int)width
				<< " max_symbol_width=" << bits::hi(max_symbol) + 1 << " n_width=" << bits::hi(n)
				<< std::endl;
	...
	void sort(tIV& sa, const char* file_name, uint8_t num_bytes)
	{
		...
		DBG_OUT << "sizeof(int_vector<>::difference_type)=" << sizeof(int_vector<>::difference_type)
				<< std::endl;
